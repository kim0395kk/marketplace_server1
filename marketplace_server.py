    ...
    def run_steps(self, steps, status_callback=None):
        ...
        while i < len(steps) and self.is_running:
            ...
            if stype == "joint_start":
                ...
                if data_list:
                    loop_stack.append(
                        {"start": i, "data": data_list, "idx": 0, "is_dict": is_dict}
                    )
                    item = data_list[0]
                    if is_dict:
                        # 엑셀의 컬럼명 → context 키
            ...
            # ----- 2) 직접 입력 반복 (줄목록) -----
                ...
                if lines:
                    loop_stack.append(
                        {"start": i, "data": lines, "idx": 0, "is_dict": False}
                    )
                    self.context["i"] = lines[0]
                    if status_callback:
                        status_callback(f"📝 직접입력 반복 시작 (총 {len(lines)}줄)")
                else:
                    if status_callback:
            ...
            # ----- 3) 반복 종료 -----
                ...
                if loop_stack:
                    ...
                    if curr["idx"] < len(curr["data"]):
                        ...
                        if status_callback:
                            status_callback(
                                f"🔁 다음 반복 진행 ({curr['idx']+1}/{len(curr['data'])})"
                            )
                        # 반복 블록 시작 위치로 점프
                        i = curr["start"]
                    else:
...
class App:
    ...
    def show_prop(self, step):
        ...
        if t == "click":
            ...
            self.lbl_prop_title.config(text="입력할 내용 (예: {i} 사용 가능):")
        elif t == "wait":
    ...
    def sell_macro(self):
        ...
        name = self.entry_macro_name.get().strip()
        if not name and not self.current_macro:
            messagebox.showwarning("경고", "판매할 부품이 없습니다.\n부품 이름을 입력하거나 부품을 편집하세요.")
            return
        
        if not name:
            name = simpledialog.askstring("부품 이름", "판매할 부품 이름을 입력하세요:")
            if not name:
                return
        ...
        tk.Label(top, text="설명:", pady=4).pack(anchor="w", padx=10, pady=(10, 0))
        text_desc = tk.Text(top, width=40, height=6)
        text_desc.pack(fill="both", expand=True, padx=10, pady=2)
        
        tk.Label(top, text="가격 (포인트, 0이면 무료):", pady=4).pack(anchor="w", padx=10, pady=(10, 0))
        entry_price = tk.Entry(top, width=40)
        entry_price.insert(0, "0")
        entry_price.pack(fill="x", padx=10, pady=2)
        
        def _ok():
            description = text_desc.get("1.0", "end-1c").strip()
            try:
                price = int(entry_price.get().strip() or "0")
            except:
                price = 0
            
            metadata = {
                "author": self.marketplace.user_id or "",
            ...
            try:
                steps = self.current_macro if self.current_macro else self.engine.macros.get(name, [])
                if not self.engine.export_macro(name, steps, temp_zip, metadata):
                    messagebox.showerror("오류", "부품 준비에 실패했습니다.")
                    return
                
                # 웹 게시판에 업로드
                success, msg = self.marketplace.upload_item("macro", name, temp_zip, metadata)
                if success:
                    messagebox.showinfo("성공", f"부품 '{name}' 판매 등록 완료!\n\n{msg}\n포인트: {self.marketplace.points}P")
                    # 포인트 표시 업데이트
                    self._update_points_display()
                else:
                    messagebox.showerror("오류", f"판매 등록 실패:\n{msg}")
            except Exception as e:
                messagebox.showerror("오류", f"판매 등록 중 오류가 발생했습니다:\n{e}")
            finally:
                if os.path.exists(temp_zip):
                    os.remove(temp_zip)
        
        tk.Button(top, text="확인", command=_ok, bg="#0969da", fg="white").pack(pady=10)
    ...
    def sell_job(self):
        ...
        name = self.entry_job_name.get().strip()
        if not name and not self.current_job_steps:
            messagebox.showwarning("경고", "판매할 조립품이 없습니다.\n작업 이름을 입력하거나 조립라인을 구성하세요.")
            return
        
        if not name:
            name = simpledialog.askstring("작업 이름", "판매할 조립품 이름을 입력하세요:")
            if not name:
                return
        ...
        def _ok():
            ...
            try:
                steps = self.current_job_steps if self.current_job_steps else self.engine.jobs.get(name, [])
                if not self.engine.export_job(name, steps, temp_zip, metadata):
                    messagebox.showerror("오류", "조립품 준비에 실패했습니다.")
                    return
                
                # 웹 게시판에 업로드
                success, msg = self.marketplace.upload_item("job", name, temp_zip, metadata)
                if success:
                    messagebox.showinfo("성공", f"조립품 '{name}' 판매 등록 완료!\n\n{msg}\n포인트: {self.marketplace.points}P")
                    self._update_points_display()
                else:
                    messagebox.showerror("오류", f"판매 등록 실패:\n{msg}")
            except Exception as e:
